...
function Algorithms() {
  ...
  const handleDelete = async (algorithmId: number) => {
    setError(null);
    try {
      await apiDelete(`/v1/algorithms/${algorithmId}`);
      if (editingAlgorithmId === algorithmId) {
        cancelEdit();
      }
      loadAlgorithms();
    } catch (err: any) {
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {algorithms.map((algo) => (
              <tr key={algo.id} className="hover:bg-slate-50">
                <td className="px-3 py-2">
                  <div className="text-sm font-semibold text-slate-900">{algo.code}</div>
