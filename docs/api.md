...
- Training result create, list, detail, and patch payloads include optional `comments` free text for run-level review notes.
- Use `PATCH /v1/training-results/{result_id}` to edit result metadata: `model_id`, `run_source`, `comments`, `status`, `run_started_at`, `run_completed_at`, `data_from`, `data_to`, `summary_json`, and `chart_series_json`. Summary/chart JSON sent to this endpoint replaces the saved metadata object.
- Use `POST /v1/training-results/{result_id}/artifacts` to append artifacts to an existing result. The payload may include `summary_json`, `chart_series_json`, `data_from`, and `data_to`; summary/chart JSON is merged into the existing result metadata, while provided date bounds replace the result date bounds.
