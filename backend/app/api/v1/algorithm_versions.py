...
from app.schemas import AlgorithmVersionCreate, AlgorithmVersionRead
...
def create_version(algo_id: int, payload: AlgorithmVersionCreate, db: Session = Depends(get_db)):
    ...
    return crud.create_algorithm_version(db, algo_id=algo_id, version=payload)


@router.get("/version/{version_id}", response_model=AlgorithmVersionRead)
...
@router.patch("/version/{version_id}", response_model=AlgorithmVersionRead)
def patch_version(version_id: int, payload: AlgorithmVersionCreate, db: Session = Depends(get_db)):
    version = crud.get_algorithm_version(db, version_id=version_id)
    if not version:
        raise HTTPException(status_code=404, detail="Version not found")
...
@router.post("/version/{version_id}/make-current", response_model=AlgorithmVersionRead)
